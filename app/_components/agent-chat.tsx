 ...
 * serialised JSON header value.  Returns `undefined` when no config has
 * been saved yet, so the header is omitted and the server uses its
 * fallback.
 */
function readModelConfigHeader(): Record<string, string> {
  try {
    ...
    // Only send the fields that the server needs — omit empty values.
